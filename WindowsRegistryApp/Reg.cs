...
namespace WindowsRegistryApp
{
  class Reg
  {
    ...
    static public List<string> regValueKindsList = Enum.GetValues(typeof(RegistryValueKindNative)).Cast<RegistryValueKindNative>().Where(x => x > 0).Select(x => x.ToString()).ToList();
